...
function PoolsPage({ wallet, onWalletChange, poolState }) {
  ...
  const tokensInPool = useMemo(() => {
    ...
    const pushToken = (candidate) => {
      ...
      const exists = tokens.some((entry) => symbolsEqual(entry.symbol, candidate.symbol));
      if (exists) {
        return;
      }
      tokens.push(withTokenLogo(candidate));
    };

    pushToken(TOKENS.KTA);

    if (poolData) {
      (poolData.tokens || []).forEach(pushToken);
      if (poolData.baseToken?.symbol) {
        const key = poolData.baseToken.symbol;
        const reserve = poolData.reserves?.[key];
        pushToken({
  ...
  const tokenA = tokensInPool[0] || TOKENS.KTA;
